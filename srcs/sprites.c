...
void				init_sprites(t_env *env, char **textures, int n)
{
	...
	while (++c.y < env->map.size.y)
	{
		...
		while (++c.x < env->map.size.x)
			if (env->map_tiles[c.y][c.x].value < 0)
			{
				env->sprites[env->n_sprites].location =
											(t_coord_f){c.x + .5f, c.y + .5f};
				env->sprites[env->n_sprites].type =
...
void				render_sprites(t_env *env, t_rendering_layer *layer)
{
	...
	{
		angle0_x = atan2f(env->sprites[i].location.y - env->player.location.y,
			env->sprites[i].location.x - env->player.location.x) -
			env->player.angle;
		...
		while (++c.x < c_max.x)
		{
																...
																WIDTH * HEIGHT)
			{
				c_tex.x = (float)(c.x - c_topleft.x) /
					...
					(env->scene.bmp_sprite[env->sprites[i].type].dim.x - 2);
				c_tex.y = (float)(c.y - c_bottomright.y) /
					(c_topleft.y - c_bottomright.y) *
					(env->scene.bmp_sprite[env->sprites[i].type].dim.y - 2);
				layer->ij = c;
				layer->uv = c_tex;
