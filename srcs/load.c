...
t_tile			**get_map(char *file, t_env *env)
{
	...
	if (!(input = extract_input(file, &env->map_y)) || env->map_y <= 0 ||
				(!(tile = (t_tile **)malloc(env->map_y * sizeof(t_tile *)))))
		return (NULL);
	i = 0;
	old_size_x = -1;
	ptr = input;
	while (input)
	{
		if (!(tile[i++] = create_tab(input->content, &env->map_x)))
			return (NULL);
		if (old_size_x < 0)
			old_size_x = env->map_x;
		else if (old_size_x != env->map_x)
			return (NULL);
		input = input->next;
	}
	ft_lstdel(&ptr, &del);
